...
const generateDailyViewsData = () => {
  ...
  for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
    ...
    // Estimate views distribution from total views equally
    const viewsForDate = selectedForms.reduce((total, form) => {
      try {
        const formCreatedAt = startOfDay(parseISO(form.createdAt))
        const now = startOfDay(new Date())
        const daysSinceCreation = Math.max(
          1,
          Math.floor((Math.min(+now, +endDate) - +formCreatedAt) / (1000 * 60 * 60 * 24)) + 1
        )

        if (d >= formCreatedAt && d <= now) {
          return total + Math.floor((form.views || 0) / daysSinceCreation)
        }
        return total
      } catch {
        return total
