...
const FormPreview: React.FC<FormPreviewProps> = ({ form, inBuilder = false, onClose }) => {
  ...
  // Function to validate file type - FIXED VERSION
  const isValidFileType = (file: File, acceptedTypes: string): boolean => {
    if (!acceptedTypes || acceptedTypes.trim() === "") return true
    ...
    console.log("Validating file:", { fileName, fileType, fileExtension, allowedTypes })
    ...
    return allowedTypes.some((type) => {
      ...
      if (cleanType.startsWith(".")) {
        return fileExtension === cleanType.substring(1)
      }
          ...
          <div className="mb-6" key={element.id}>
                ...
                if (file) {
                  ...
                  if (file.size > maxFileSize * 1024 * 1024) {
                    toast({
                      ...
                      description: `File size must be less than ${maxFileSize}MB. Selected file is ${(file.size / (1024 * 1024)).toFixed(2)}MB`,
                      variant: "destructive",
                    })
                    e.target.value = ""
                    return
                  ...
                  // Check file type if specified - STRICT VALIDATION
                  if (acceptedTypes && !isValidFileType(file, acceptedTypes)) {
                    toast({
                      title: "Invalid file type",
                      description: `Only ${acceptedTypes} files are allowed. Selected: ${file.type || file.name.split(".").pop()}`,
                      variant: "destructive",
                    })
                    e.target.value = ""
                    return
            ...
            {(element.properties?.helpText || acceptedTypes || maxFileSize) && (
              <div className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                ...
                {acceptedTypes && (
                  <p>
                    <strong>Accepted file types:</strong> {acceptedTypes}
                  </p>
                )}
                <p>
